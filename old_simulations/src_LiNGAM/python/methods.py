...
def graphical_rica(latent, observed, g, data, data_whitening, epochs, lr, W_w, w_init, w_true, momentum=0, lmbda=0):
    ...
    loss_data = torch.zeros(epochs, dtype=torch.float64)
    w_loss_data = torch.zeros(len(w_true), epochs, dtype=torch.float64)
    ...
    min_loss = None
    for epoch in range(epochs):
        adj = torch.eye(len(g.nodes()), dtype=torch.float64)
        if w_init == 'cov_guess':
